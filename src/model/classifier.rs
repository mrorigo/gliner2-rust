...
impl ClassifierHead {
    pub fn new(hidden_size: usize, device: Device) -> Result<Self> {
        ...
        let layer1 = candle_nn::linear(hidden_size, hidden_size * 2, vb.pp("0"))
            .map_err(|e| GlinerError::model_loading(format!("Failed to create classifier layer1: {e}")))?;
        let layer2 = candle_nn::linear(hidden_size * 2, 1, vb.pp("2"))
            .map_err(|e| GlinerError::model_loading(format!("Failed to create classifier layer2: {e}")))?;

        Ok(Self { hidden_size, layer1, layer2, device })
    }
    ...
    pub fn from_var_builder(vb: VarBuilder, config: &ExtractorConfig, device: Device) -> Result<Self> {
        ...
        let layer1 = candle_nn::linear(config.hidden_size, config.hidden_size * 2, vb.pp("0"))
            .map_err(|e| GlinerError::model_loading(format!("Failed to load classifier layer1: {e}")))?;
        let layer2 = candle_nn::linear(config.hidden_size * 2, 1, vb.pp("2"))
            .map_err(|e| GlinerError::model_loading(format!("Failed to load classifier layer2: {e}")))?;

        Ok(Self { hidden_size: config.hidden_size, layer1, layer2, device })
    }

    pub fn forward(&self, x: &Tensor) -> Result<Tensor> {
        let x = self.layer1.forward(x)
            .map_err(|e| GlinerError::model_loading(format!("Classifier layer1 forward failed: {e}")))?;
        let x = x.relu()
            .map_err(|e| GlinerError::model_loading(format!("Classifier ReLU failed: {e}")))?;
        let logits = self.layer2.forward(&x)
            .map_err(|e| GlinerError::model_loading(format!("Classifier layer2 forward failed: {e}")))?;
        // Squeeze last dim: (..., 1) -> (...)
