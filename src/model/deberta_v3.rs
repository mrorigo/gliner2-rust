...
//! - Standard multi-head attention (not disentangled)
...
/// Self-attention with relative position bias (DeBERTa V3 style)
...
impl DebertaV3Attention {
    ...
    fn forward(&self, hidden_states: &Tensor, attention_mask: &Tensor, rel_embeddings: Option<&Tensor>) -> Result<Tensor> {
        ...
        let new_shape = &[batch_size as i64, seq_len as i64, self.num_attention_heads as i64, self.attention_head_size as i64];
        let query = query.reshape((batch_size, seq_len, self.num_attention_heads, self.attention_head_size))?.transpose(1, 2)?;
        let key = key.reshape((batch_size, seq_len, self.num_attention_heads, self.attention_head_size))?.transpose(1, 2)?;
        let value = value.reshape((batch_size, seq_len, self.num_attention_heads, self.attention_head_size))?.transpose(1, 2)?;
        
        // Attention scores: (batch, heads, seq, head_size) @ (batch, heads, head_size, seq) -> (batch, heads, seq, seq)
        let attention_scores = query.matmul(&key.transpose(2, 3)?)?;
        let attention_scores = (attention_scores / (self.attention_head_size as f64).sqrt())?;
        
        // Apply attention mask - broadcast (batch, 1, 1, seq) to (batch, heads, seq, seq)
