...
impl CountPredictionLayer {
    pub fn new(hidden_size: usize, max_count: usize, device: Device) -> Result<Self> {
        ...
        let layer1 = candle_nn::linear(hidden_size, hidden_size * 2, vb.pp("0")).map_err(|e| {
            GlinerError::model_loading(format!("Failed to create count_pred layer1: {e}"))
        })?;
        let layer2 = candle_nn::linear(hidden_size * 2, max_count, vb.pp("2")).map_err(|e| {
            GlinerError::model_loading(format!("Failed to create count_pred layer2: {e}"))
        })?;
    ...
    pub fn from_var_builder(
        ...
        let layer1 = candle_nn::linear(config.hidden_size, config.hidden_size * 2, vb.pp("0"))
            .map_err(|e| {
                GlinerError::model_loading(format!("Failed to load count_pred layer1: {e}"))
            })?;
        let layer2 =
            candle_nn::linear(config.hidden_size * 2, max_count, vb.pp("2")).map_err(|e| {
                GlinerError::model_loading(format!("Failed to load count_pred layer2: {e}"))
            })?;
    ...
    pub fn predict_count(&self, schema_emb: &Tensor) -> Result<CountPredictionOutput> {
        ...
        let logits = self.layer2.forward(&x).map_err(|e| {
            GlinerError::model_loading(format!("CountPred layer2 forward failed: {e}"))
        })?;
        ...
        let count = argmax
            .to_scalar::<u32>()
            .map_err(|e| GlinerError::model_loading(format!("CountPred to_scalar failed: {e}")))?
            as usize;
