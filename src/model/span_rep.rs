...
impl SpanRepresentationLayer {
    ...
    fn build_from_varbuilder(
        ...
        // project_start: Linear(768→3072) + GELU + Linear(3072→768)
        let project_start_0 =
            candle_nn::linear(hidden_size, intermediate, vb.pp("project_start").pp("0")).map_err(
                |e| GlinerError::model_loading(format!("Failed to create project_start_0: {e}")),
            )?;
        let project_start_1 =
            candle_nn::linear(intermediate, hidden_size, vb.pp("project_start").pp("3")).map_err(
                |e| GlinerError::model_loading(format!("Failed to create project_start_1: {e}")),
            )?;

        // project_end: Linear(768→3072) + GELU + Linear(3072→768)
        let project_end_0 =
            candle_nn::linear(hidden_size, intermediate, vb.pp("project_end").pp("0")).map_err(
                |e| GlinerError::model_loading(format!("Failed to create project_end_0: {e}")),
            )?;
        let project_end_1 =
            candle_nn::linear(intermediate, hidden_size, vb.pp("project_end").pp("3")).map_err(
                |e| GlinerError::model_loading(format!("Failed to create project_end_1: {e}")),
            )?;

        // out_project: Linear(1536→3072) + GELU + Linear(3072→768)
        let out_project_0 =
            candle_nn::linear(hidden_size * 2, intermediate, vb.pp("out_project").pp("0"))
                .map_err(|e| {
                    GlinerError::model_loading(format!("Failed to create out_project_0: {e}"))
                })?;
        let out_project_1 =
            candle_nn::linear(intermediate, hidden_size, vb.pp("out_project").pp("3")).map_err(
                |e| GlinerError::model_loading(format!("Failed to create out_project_1: {e}")),
            )?;
    ...
    pub fn forward(&self, token_embs: &Tensor) -> Result<SpanRepOutput> {
        ...
        for width in 0..self.max_width {
            ...
            if end_idx == 0 {
                let zeros = Tensor::zeros((seq_len, self.hidden_size), DType::F32, &self.device)
                    .map_err(|e| {
                        GlinerError::model_loading(format!("Failed to create zeros: {e}"))
                    })?;
                span_reps.push(zeros);
                continue;
            }

            let start_tokens = token_embs
                .narrow(0, 0, end_idx)
                .map_err(|e| GlinerError::model_loading(format!("Failed to narrow start: {e}")))?;
            let end_tokens = token_embs
                .narrow(0, width, end_idx)
            ...
            let combined = combined.relu().map_err(|e| {
                GlinerError::model_loading(format!("Failed to apply pre-out-project relu: {e}"))
            })?;
            ...
            if span_rep.dims()[0] < seq_len {
                ...
                let padding = Tensor::zeros((pad_size, self.hidden_size), DType::F32, &self.device)
                    .map_err(|e| {
                        GlinerError::model_loading(format!("Failed to create padding: {e}"))
                    })?;
        ...
        let spans_idx =
            Tensor::from_slice(&spans_idx_data, (seq_len, self.max_width, 2), &self.device)
                .map_err(|e| {
                    GlinerError::model_loading(format!("Failed to create spans_idx: {e}"))
                })?;
        let span_mask =
            Tensor::from_slice(&span_mask_data, (seq_len, self.max_width), &self.device).map_err(
                |e| GlinerError::model_loading(format!("Failed to create span_mask: {e}")),
            )?;
